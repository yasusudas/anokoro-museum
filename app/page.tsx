...
type Exhibit = { id: string; number: string; title: string; subtitle: string; category: string; years: string; memory: string; count: number; theme: string };
...
export default function Home() {
  ...
  return (
    <main className="museum-shell">
      ...
      <div className="corridor-wrap">
        ...
        <div className="corridor" ref={corridorRef}>
          ...
          <section className="gallery" aria-live="polite">
            {visible.map((exhibit) => <article className="exhibit" key={exhibit.id}><div className="spotlight" /><button className="frame" onClick={() => setSelected(exhibit)} aria-label={`${exhibit.title}の詳細を見る`}><span className="frame-inner"><ExhibitArt theme={exhibit.theme} /></span></button><div className="exhibit-label"><span className="item-number">{exhibit.number}</span><div><h2>{exhibit.title}</h2><p>{exhibit.subtitle}</p><small>{exhibit.years} 生まれの記憶</small></div><button className={liked.includes(exhibit.id) ? "nostalgia liked" : "nostalgia"} onClick={() => toggleLike(exhibit.id)} aria-label="しんみりする"><NostalgiaIcon /><b>{exhibit.count + (liked.includes(exhibit.id) ? 1 : 0)}</b><small>しんみり</small></button></div></article>)}
      ...
      {selected && <div className="modal-backdrop" onMouseDown={() => setSelected(null)}><section className="detail-modal" role="dialog" aria-modal="true" aria-label={`${selected.title}の展示詳細`} onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelected(null)} aria-label="閉じる">×</button><div className="modal-art"><div className="frame modal-frame"><span className="frame-inner"><ExhibitArt theme={selected.theme} /></span></div></div><div className="modal-content"><p className="eyebrow">EXHIBIT {selected.number} · {selected.category}</p><h2>{selected.title}</h2><p className="modal-subtitle">{selected.subtitle}</p><p className="modal-memory">{selected.memory}</p><div className="memory-tag">主に <b>{selected.years}年生まれ</b> の記憶</div><button className={liked.includes(selected.id) ? "modal-like liked" : "modal-like"} onClick={() => toggleLike(selected.id)}><NostalgiaIcon />しんみりした <b>{selected.count + (liked.includes(selected.id) ? 1 : 0)}</b></button><div className="thread-preview"><span>みんなの思い出</span><p>「これ、学校帰りによく友達と話してたなあ…」</p><button>思い出を読む →</button></div></div></section></div>}
